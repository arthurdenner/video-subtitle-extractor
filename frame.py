...
def getTotalFrames(videoFile):
    ...
    frameRate = int(math.ceil(cap.get(cv2.CAP_PROP_FPS))) #frame rate
    totalFrames = int(math.ceil(cap.get(cv2.CAP_PROP_FRAME_COUNT))) #frame ratecap.get(cv2.CV_CAP_PROP_FRAME_COUNT)
    return int(math.floor(totalFrames/frameRate))